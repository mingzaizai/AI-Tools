...
const MergeView: React.FC<MergeViewProps> = ({ images, onUpload }) => {
  ...
  return (
    <div className="h-full flex flex-col md:flex-row bg-[#f8fafc]">
      <div className="flex-1 flex flex-col min-w-0 border-r border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-bold text-slate-800 text-sm">选择合并素材 (已选 {selectedIds.length}/4)</h3>
          </div>
        ...
        <div className="flex-[2] bg-white border-t border-slate-200 p-8 flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="w-full md:w-80 bg-white shrink-0 flex flex-col overflow-hidden">
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => setDirection('horizontal')} className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border text-xs font-bold transition-all ${direction === 'horizontal' ? 'bg-indigo-600 border-indigo-400 text-white' : 'bg-white border-slate-200 text-slate-700'}`}><ArrowRight className="w-4 h-4" /> 左右</button>
                  <button onClick={() => setDirection('vertical')} className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border text-xs font-bold transition-all ${direction === 'vertical' ? 'bg-indigo-600 border-indigo-400 text-white' : 'bg-white border-slate-200 text-slate-700'}`}><ArrowDown className="w-4 h-4" /> 上下</button>
                </div>
              </div>
